import {
  ...
  TimelineEvent, OutcomeBreakdown, ROIScore, ActionPlan, SkillMatch,
} from "./types";
...
// ─── Rule-based skill match (fallback when no LLM key) ───────────────────────
...
const SKILL_KEYWORDS = [
  ...
  // Practices / concepts
  "rest", "graphql", "grpc", "microservices", "agile", "scrum", "tdd", "machine learning",
  "deep learning", "pytorch", "tensorflow", "data science", "llm", "nlp",
];

export function analyzeSkillMatch(jobDescription: string, resumeText: string): SkillMatch {
  const jdLower = jobDescription.toLowerCase();
  const resumeLower = resumeText.toLowerCase();

  const jobSkills = SKILL_KEYWORDS.filter((k) => jdLower.includes(k));
  const matchedSkills = jobSkills.filter((k) => resumeLower.includes(k));
  const missingSkills = jobSkills.filter((k) => !resumeLower.includes(k));
  ...
  let verdict: string;
  if (fitScore >= 75)
    verdict = "Your technical background aligns strongly with this role's requirements.";
  else if (fitScore >= 50)
    verdict = "You meet the core requirements but have gaps in a few key areas.";
  else if (fitScore >= 25)
    verdict = "Significant skill gaps exist — consider upskilling before applying.";
  else
    verdict = "Your current skill set is a poor match for the listed requirements.";

  return {
    fitScore,
    matchingSkills: matchedSkills.slice(0, 6).map(capitalise),
    missingSkills: missingSkills.slice(0, 4).map(capitalise),
    verdict,
    worthApplying: fitScore >= 50,
  };
}
